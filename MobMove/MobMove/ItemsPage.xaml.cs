...
namespace MobMove
{
    public partial class ItemsPage : ContentPage
    {
        ...
        private void TestButton1_Clicked(object sender, EventArgs e)
        {
            ...
            if (myItems.Count == 0)
            {
                myItems.Add(new BoxItem { Id = 1, Name = "Brass Knuckles", Quantity = "6", InBox = "Foyer", Photo = "MugShot", Description = "Well used" });
                myItems.Add(new BoxItem { Id = 2, Name = "Tommy Gun", Quantity = "2", InBox = "Lounge", Photo = "MugShot", Description = "Locked and loaded" });
                myItems.Add(new BoxItem { Id = 3, Name = "Teddy Bear", Quantity = "1", InBox = "Master Bedroom", Photo = "MugShot", Description = "Touch it, and you will get cement galoshes!" });
            }
            //load it
        ...
        {
            int idOfSelected = (int)((MenuItem)sender).CommandParameter;
            var selectedData = myItems.Where(x => x.Id == idOfSelected).ToList<BoxItem>();
            string extrainfo = selectedData[0].Description;

            DisplayAlert("MoreInfo", extrainfo, "OK");
